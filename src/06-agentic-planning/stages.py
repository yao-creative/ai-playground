...
from models import DraftResult, PlanResult
...
class PlanRedrafter(StageJSONMixin):
    def __init__(self, *, client, model: str) -> None:
        self.client = client
        self.model = model
    ...
    def run(
        ...
        *,
        ...
        prompt = "\n".join(
            [
                "You are a plan redrafting stage for a grounded assistant.",
                "Return JSON only with keys: objective, evidence_needed, proposed_actions.",
                "Revise the plan to address the user's feedback without inventing evidence.",
                f"User question: {user_input}",
                f"Prior plan: {prior_plan}",
        ...
        objective = str(payload.get("objective", prior_plan.objective)).strip()
        evidence_needed_raw = payload.get("evidence_needed", prior_plan.evidence_needed)
        proposed_actions_raw = payload.get("proposed_actions", prior_plan.proposed_actions)

        evidence_needed = [str(item) for item in evidence_needed_raw] if isinstance(evidence_needed_raw, list) else []
        proposed_actions = [str(item) for item in proposed_actions_raw] if isinstance(proposed_actions_raw, list) else []

        return PlanResult(
            objective=objective or prior_plan.objective,
            evidence_needed=evidence_needed or list(prior_plan.evidence_needed),
            proposed_actions=proposed_actions or list(prior_plan.proposed_actions),
        )
